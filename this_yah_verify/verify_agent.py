...
def duckduckgo_search(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """Search DuckDuckGo and return list of result dictionaries."""
    results = []
    try:
        with DDGS() as ddgs:
            for r in ddgs.text(query, max_results=max_results):
                results.append({'title': r.get('title'), 'href': r.get('href')})
    except Exception as err:  # network or backend errors
        logging.error(f"DuckDuckGo search failed: {err}")
    return results
...
def archive_search(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """Search Archive.org for the query."""
    url = (
        "https://archive.org/advancedsearch.php?q="
        f"{requests.utils.quote(query)}&output=json&rows={max_results}"
    )
    results = []
    try:
        ...
        for doc in data.get("response", {}).get("docs", []):
            ...
            if identifier:
                results.append(
                    {
                        "title": title,
                        "href": f"https://archive.org/details/{identifier}",
                    }
                )
    except Exception as err:
        logging.error(f"Archive.org search failed: {err}")
    return results
...
def verify_media(url: str) -> Dict[str, str]:
    """Attempt to verify a media URL via InVID service."""
    invid_url = (
        "https://weverify.eu/invid-verification-plugin/?url="
        f"{requests.utils.quote(url)}"
    )
    try:
        res = requests.get(invid_url, timeout=10)
        if res.ok:
            return {"status": "Checked", "url": invid_url}
        return {"status": "Error", "note": f"HTTP {res.status_code}"}
    except Exception as err:
        logging.error(f"Media verification failed: {err}")
        return {"status": "Unavailable", "note": str(err)}
...
def build_report(
    ...
    media_result: Optional[Dict[str, str]] = None,
) -> Dict[str, any]:
    ...
    if media_result:
        report["Media Verification"] = media_result
    return report
...
def main():
    ...
    parser.add_argument('--media-url', help='URL of media (image/video) to verify')
    ...
    media_result = None
    if args.media_url:
        logging.info('Verifying media...')
        media_result = verify_media(args.media_url)

    report = build_report(args.claim, evidence, fact_checks, media_result)
